...
function centerSel(sel){
	$.ajax({
		url : '/userDetail/centerSelectLeveltest',			
		type : 'post',
		data : {
			'center_no': $(sel).val()
		},
...
function groupSel(sel){
	$.ajax({
		url : '/userDetail/groupSelectLeveltest',			
		type : 'post',
		data : {
...
function setTutor(){
	...
	console.log("center_no", $("#center_no").val());
	console.log("user_teacher_no", $("#user_teacher_no").val());

	$.ajax({
		url : '/userDetail/modifyLevel',			
		type : 'post',
		data : {
			...
			"group_no": $("#teacher_group_no").val(),
			"user_teacher_no": $("#user_teacher_no").val(),
			"leveltest_no": $("#leveltest_no").val()
		},
		success : function() {
			alert("레벨테스트가 변경되었습니다.");
